/**
 * Barra de navegación inferior para móvil.
 *
 * En móvil: fixed en la parte inferior (como Sofascore, OneFootball).
 * En desktop: oculta (la nav está en el header).
 *
 * Preparada para añadir más secciones — solo añade un item al array.
 */

'use client'

import Link from 'next/link'
import { usePathname } from 'next/navigation'

const NAV_ITEMS = [
  {
    href: '/',
    label: 'Partidos',
    icon: (active: boolean) => (
      <svg
        viewBox='0 0 24 24'
        className={`w-6 h-6 ${active ? 'text-green-400' : 'text-gray-500'}`}
        fill='none'
        stroke='currentColor'
        strokeWidth={active ? 2.5 : 1.8}
      >
        {/* Balón de fútbol simplificado */}
        <circle
          cx='12'
          cy='12'
          r='9'
        />
        <path
          d='M12 3c0 0-2 3-2 9s2 9 2 9'
          strokeLinecap='round'
        />
        <path
          d='M3 12h18'
          strokeLinecap='round'
        />
        <path
          d='M5 7c1.5 1 4 1.5 7 1.5S17.5 8 19 7'
          strokeLinecap='round'
        />
        <path
          d='M5 17c1.5-1 4-1.5 7-1.5S17.5 16 19 17'
          strokeLinecap='round'
        />
      </svg>
    )
  },
  {
    href: '/standings',
    label: 'Clasificación',
    icon: (active: boolean) => (
      <svg
        viewBox='0 0 24 24'
        className={`w-6 h-6 ${active ? 'text-green-400' : 'text-gray-500'}`}
        fill='none'
        stroke='currentColor'
        strokeWidth={active ? 2.5 : 1.8}
      >
        {/* Tabla / clasificación */}
        <rect
          x='3'
          y='3'
          width='18'
          height='18'
          rx='2'
        />
        <path
          d='M3 9h18M3 15h18M9 9v9'
          strokeLinecap='round'
        />
      </svg>
    )
  }
  // Aquí irán más secciones en el futuro:
  // { href: '/favorites', label: 'Favoritos', icon: ... },
  // { href: '/profile',   label: 'Perfil',    icon: ... },
]

export default function BottomNav() {
  const pathname = usePathname()

  return (
    /*
      md:hidden → solo visible en móvil.
      fixed bottom-0 → pegada a la parte inferior de la pantalla.
      z-50 → por encima de todo el contenido.
      safe-area-inset-bottom → respeta el notch / home indicator del iPhone.
    */
    <nav
      className='fixed bottom-0 left-0 right-0 md:hidden z-50 bg-gray-900/95 backdrop-blur-sm border-t border-gray-800'
      style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}
    >
      <div className='flex items-stretch'>
        {NAV_ITEMS.map(item => {
          // Activo si la ruta actual empieza por el href del item
          // Excepción: '/' solo es activo si es exactamente '/'
          const active = item.href === '/' ? pathname === '/' : pathname.startsWith(item.href)

          return (
            <Link
              key={item.href}
              href={item.href}
              className={`
                flex-1 flex flex-col items-center justify-center gap-1 py-2.5
                transition-colors duration-150
                ${active ? 'text-green-400' : 'text-gray-500 hover:text-gray-300'}
              `}
            >
              {item.icon(active)}
              <span className={`text-xs font-medium ${active ? 'text-green-400' : ''}`}>{item.label}</span>
            </Link>
          )
        })}
      </div>
    </nav>
  )
}
